...
webpush.setVapidDetails('mailto:admin@lunatracker.local', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
// ─── sendToAll ────────────────────────────────────────────────────────────────
...
export async function sendToAll(payload) {
    ...
    if (subscriptions.length === 0)
        return;
    const results = await Promise.allSettled(subscriptions.map(sub => sendOne(sub, payload)));
